...
class MODE:
    ...
    def evaluate_objective_functions(self, individual):
        x = individual['position'][0]
        s_t = individual['position'][1]
    ...
    def generate_mutant_vector(self, donor_indices, scaling_factor):
        donor_vectors = [self.population[i]['position'][0] for i in donor_indices]
        ...
        donor_vectors = [self.population[i]['position'][1] for i in donor_indices]
        base_vector = donor_vectors[0]
        differential_vector = np.subtract(donor_vectors[1], donor_vectors[2])
        time_mutant_vector = base_vector + scaling_factor * differential_vector

        if any(member>148 or member<1 for member in time_mutant_vector):
            time_mutant_vector = [((x - np.min(time_mutant_vector))*147 / (np.max(time_mutant_vector) - np.min(time_mutant_vector))+1) for x in time_mutant_vector]
        time_mutant_vector = sorted(list((np.round(time_mutant_vector)).astype(int)))
        return mutant_vector, time_mutant_vector
    ...
    def select_next_generation(self, target_index, trial_vector, time):
        target_individual = self.population[target_index]        
        objectives, states = self.evaluate_objective_functions({'position':[trial_vector, time] })
        trial_individual = {'position': [trial_vector, time], 'objective_values': objectives, 'states': states}


        if self.is_dominating(trial_individual, target_individual):
            self.population[target_index] = trial_individual
            # self.update_archive(trial_individual)
            ...
            # if selected_individual['position'] == trial_individual['position']:
                # self.update_archive(trial_individual)

    # def update_archive(self, individual):
    #     if not any(all(individual['objective_values'][i] > p['objective_values'][i] for i in range(2)) for p in self.archive):
    #         new_archive = [archived_individual for archived_individual in self.archive if not self.is_dominating(individual, archived_individual)]
    ...
    def run(self):
        ...
        for _ in range(self.max_generations):
            for target_index, target_individual in enumerate(self.population):
                ...
                mutant_vector, time_mutant_vector = self.generate_mutant_vector(donor_indices, self.scaling_factor)
                trial_vector = self.perform_crossover(target_individual['position'][0], mutant_vector, self.crossover_prob)
                self.select_next_generation(target_index, trial_vector, time_mutant_vector)
...
u = best_individual['position'][0]
s_t = best_individual['position'][1]
